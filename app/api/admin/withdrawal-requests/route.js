...
// Mock withdrawal request data for demonstration
const generateMockWithdrawalRequests = (count = 20) => {
  const mockRequests = [];
  ...
  for (let i = 0; i < count; i++) {
    ...
    mockRequests.push({
      ...
      gateway: ['Crypto', 'Bank Transfer', 'PayPal'][Math.floor(Math.random() * 3)],
      requestDate: new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000), // Last 7 days
      status: 'pending'
    });
  }
...
export async function GET(request) {
  try {
    ...
    let allRequests = generateMockWithdrawalRequests(0); // Empty for demonstration
