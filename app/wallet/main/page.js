    ...
    <div className="min-h-screen flex p-8" style={{background: 'linear-gradient(to bottom right, var(--default-body-bg-color) 0%, var(--theme-bg-gradient) 25%, var(--default-body-bg-color) 100%)', fontFamily: 'var(--default-font-family)'}}>
      ...
      <div className="flex-1">
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-white mb-2 gradient-text-enhanced animate-fadeInUp">Wallet</h1>
          <p className="text-lg animate-fadeInUp" style={{color: 'rgba(255, 255, 255, 0.7)', animationDelay: '0.2s'}}>PICZEL Wallet Balance</p>
        </div>

        {/* Wallet Balance Cards */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {/* Main Balance */}
          <div className="rounded-2xl p-6 shadow-lg transition-all duration-200" style={{
            background: 'linear-gradient(to right, rgba(var(--primary-rgb), 0.2), rgba(var(--secondary-rgb), 0.1))',
            border: '1px solid rgba(var(--primary-rgb), 0.3)'
          }}>
            <div className="text-center">
              <div className="text-sm mb-2" style={{color: 'rgba(255, 255, 255, 0.8)'}}>PICZEL Wallet Balance</div>
          ...
          <div className="rounded-2xl p-6 shadow-lg transition-all duration-200" style={{
            ...
            <div className="text-center">
              <div className="text-sm mb-2" style={{color: 'rgba(255, 255, 255, 0.8)'}}>Total Income</div>
              <div className="font-bold text-2xl" style={{color: 'rgb(var(--success-rgb))'}}>{walletData.totalIncome}</div>
            </div>
          </div>

          {/* Total Withdrawal */}
          <div className="rounded-2xl p-6 shadow-lg transition-all duration-200" style={{
            background: 'linear-gradient(to right, rgba(var(--danger-rgb), 0.2), rgba(var(--danger-rgb), 0.1))',
            border: '1px solid rgba(var(--danger-rgb), 0.3)'
          }}>
            <div className="text-center">
              <div className="text-sm mb-2" style={{color: 'rgba(255, 255, 255, 0.8)'}}>Total Withdrawal</div>
              <div className="font-bold text-2xl" style={{color: 'rgb(var(--danger-rgb))'}}>{walletData.totalWithdrawal}</div>
            </div>
          </div>

          {/* Today's Withdrawal */}
          <div className="rounded-2xl p-6 shadow-lg transition-all duration-200" style={{
            background: 'linear-gradient(to right, rgba(var(--warning-rgb), 0.2), rgba(var(--warning-rgb), 0.1))',
            border: '1px solid rgba(var(--warning-rgb), 0.3)'
          }}>
            <div className="text-center">
              <div className="text-sm mb-2" style={{color: 'rgba(255, 255, 255, 0.8)'}}>Today&apos;s Withdrawal</div>
              <div className="font-bold text-2xl" style={{color: 'rgb(var(--warning-rgb))'}}>{walletData.todaysWithdrawal}</div>
            </div>
          </div>
        </div>
