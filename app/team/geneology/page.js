...
const GenealogyPage = () => {
  ...
  // Enhanced binary tree data structure with comprehensive user info
  const treeData = {
    id: 'M001',
    name: 'John Doe',
  ...
  const handleSearch = () => {
    if (searchId.trim()) {
      // Find member by ID in the tree
      const foundMember = findMemberById(treeData, searchId);
  ...
  const TreeNode = ({ node, level = 0 }) => {
    ...
    return (
      <div className="flex flex-col items-center relative">
        ...
        >
          <div className={`w-16 h-16 md:w-20 md:h-20 rounded-full bg-gradient-to-br from-cyan-500 to-purple-600 flex items-center justify-center mb-2 shadow-lg border-3 border-white transition-all duration-300 ${
          ...
          <div className={`text-center bg-gradient-to-br from-slate-700/80 to-purple-700/80 backdrop-blur-sm px-3 py-2 md:px-4 md:py-2 rounded-lg shadow-md min-w-32 md:min-w-40 border border-purple-500/30 transition-all duration-300 ${
            isHovered ? 'scale-105 shadow-lg' : ''
          }`}>
            <div className="font-semibold text-white text-xs md:text-sm mb-1">{node.name}</div>
            <div className="text-xs text-gray-300 font-medium mb-1">{node.id}</div>
            <div className="text-xs text-cyan-400 font-medium">{node.package}</div>
            <div className="flex justify-between text-xs text-gray-400 mt-1">
              <span>L: {node.leftCount}</span>
              <span>R: {node.rightCount}</span>
            </div>
            <div className="text-xs text-green-400 font-medium mt-1">{node.business}</div>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-gradient-to-br from-slate-800/50 to-purple-800/50 backdrop-blur-sm rounded-2xl p-8 border border-purple-500/20 mb-12">
          <h2 className="text-2xl font-semibold text-white mb-4">Search Members</h2>
          ...
          {selectedMember && (
            <div className="bg-gradient-to-br from-slate-700/50 to-purple-700/50 backdrop-blur-sm p-6 rounded-xl border border-purple-500/30">
              <h3 className="text-xl font-semibold text-white mb-4">Found Member:</h3>
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="font-semibold text-white">{selectedMember.name}</div>
                  <div className="text-sm text-gray-300 font-medium">{selectedMember.id}</div>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="bg-gradient-to-br from-slate-800/50 to-purple-800/50 backdrop-blur-sm rounded-2xl p-8 border border-purple-500/20 shadow-lg">
          <h2 className="text-2xl font-semibold text-white mb-8 text-center">Team Genealogy Tree</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="font-semibold text-white text-lg">{clickedNode.name}</div>
                      <div className="text-gray-300">{clickedNode.id}</div>
                      <div className="text-cyan-400 font-medium">{clickedNode.package}</div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="text-green-400 font-medium">{clickedNode.status}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-300">Join Date:</span>
                      <span className="text-white font-medium">{clickedNode.joinDate}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-300">Sponsor ID:</span>
                      <span className="text-white font-medium">{clickedNode.sponsorId}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gradient-to-br from-slate-600/50 to-purple-600/50 rounded-lg p-3 text-center">
                      <div className="text-2xl font-bold text-cyan-400">{clickedNode.directs}</div>
                      <div className="text-xs text-gray-300">Directs</div>
                    </div>
                    <div className="bg-gradient-to-br from-slate-600/50 to-purple-600/50 rounded-lg p-3 text-center">
                      <div className="text-2xl font-bold text-purple-400">{clickedNode.leftCount + clickedNode.rightCount}</div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gradient-to-br from-slate-600/50 to-purple-600/50 rounded-lg p-3 text-center">
                      <div className="text-lg font-bold text-blue-400">{clickedNode.leftCount}</div>
                      <div className="text-xs text-gray-300">Left Count</div>
                    </div>
                    <div className="bg-gradient-to-br from-slate-600/50 to-purple-600/50 rounded-lg p-3 text-center">
                      <div className="text-lg font-bold text-orange-400">{clickedNode.rightCount}</div>
                      <div className="text-xs text-gray-300">Right Count</div>
                    </div>
                  </div>
                  
                  <div className="bg-gradient-to-br from-slate-600/50 to-purple-600/50 rounded-lg p-3 text-center">
                    <div className="text-xl font-bold text-green-400">{clickedNode.business}</div>
